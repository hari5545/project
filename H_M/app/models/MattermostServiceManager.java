...
public class MattermostServiceManager {
	...
	// Reset Password in Mattermost

	public String  resetPassword(String code, String new_password) {
		String response="";
		try {

			networkAdapter.login(getURL(MattermostConstants.USERS_LOGIN_URL), mm_user, mm_pass);
			response = networkAdapter.resetPassword(getURL(String.format(MattermostConstants.RESET_PASSWORD_USER, code)),new_password);
		}
		catch(Exception exp) {
			exp.printStackTrace();
			response = "";
	...
	public String updateUser(String...userDetails) {
		...
		try {
			networkAdapter.login(getURL(MattermostConstants.USERS_LOGIN_URL), mm_user, mm_pass);
			teamId = networkAdapter.updateUser(getURL(String.format(MattermostConstants.UPDATE_USER, userDetails[0])),userDetails[1], userDetails[2]);
		}
		catch(Exception exp) {
			exp.printStackTrace();
			teamId = "";
	...
	public String updateTab(String...tabDetails) {
		String teamId;
		String TabId = tabDetails[0];

		try {
			networkAdapter.login(getURL(MattermostConstants.USERS_LOGIN_URL), mm_user, mm_pass);
			teamId = networkAdapter.updateTab(TabId,getURL(String.format(MattermostConstants.UPDATE_CHANNEL, TabId)),tabDetails[1], tabDetails[2],tabDetails[3],tabDetails[4]);
		}
		catch(Exception exp) {
			exp.printStackTrace();
			teamId = "";
